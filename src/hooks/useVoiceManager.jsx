...
export default function GuidedSessionRunner() {
  ...
  const handleToggleSession = () => {
    if (!isSessionActive) {
      ...
      // FORCER LA DURÉE CORRECTE POUR MÉTATRON
      let duration = sessionData?.duration || 180;
      if (currentSession === 'meditation' && currentMeditation === 'metatron') {
        duration = 300; // 5 minutes exactement pour Métatron
        console.log('🌟 MÉTATRON: Durée forcée à 300 secondes');
      }
      ...
      // DÉMARRAGE VOCAL SEULEMENT POUR CERTAINES SESSIONS
      if (currentSession === 'switch' || currentSession === 'scan' || 
          (currentSession === 'meditation' && (currentMeditation === 'gratitude' || currentMeditation === 'abundance' || currentMeditation === 'metatron'))) {
        guidanceTimeoutRef.current = setTimeout(() => {
          if (voiceSettings.enabled) {
            console.log('🎤 DÉMARRAGE GUIDAGE VOCAL FORCÉ - Session:', currentSession || sessionId);
            const success = startSessionGuidance();
      ...
      // ARRÊT IMMÉDIAT ET COMPLET
      console.log('🔇 ARRÊT IMMÉDIAT ET COMPLET');
